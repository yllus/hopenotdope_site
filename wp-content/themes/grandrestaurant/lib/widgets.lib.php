	...
	function widget($args, $instance) {
		...
		if(!empty($items) && !empty($flickr_id))
		{
			...
			if(!empty($photos_arr))
			{
				...
				{
					echo '<li>';
					echo '<a class="img_frame" target="_blank" href="'.esc_url($photo['url']).'" title="'.esc_attr($photo['title']).'"><img src="'.esc_url($photo['thumb_url']).'" alt="'.esc_attr($photo['title']).'" width="75" height="75" /></a>';
					echo '</li>';
				}
